...
var MenuRoot = /** @class */ (function (_super) {
    ...
    function MenuRoot(props) {
        ...
        _this.state = {
            ...
            scroll: __assign({ sidebar: true, content: true }, _this.props.scroll)
        };
        _this.pathData = []; //存储所有可跳转路径
        _this.history = _this.props.historyType === 'hash' ? history_1.historyHash : history_1.historyBrowser;
        return _this;
    }
    MenuRoot.prototype.componentDidMount = function () {
        var _this = this;
        try {
            var _a = this.state.scroll, sidebar = _a.sidebar, content = _a.content;
            if (sidebar) {
        ...
        window.onresize = function () {
            try {
                _this.menuScroll.update();
                _this.contentScroll.update();
            }
            catch (err) {
                // IE9不支持 PerfectScrollbar
            }
            ...
            React.createElement("div", { style: {
                ...
                React.createElement("div", { className: "layout-content-xq", style: {
                    ...
                    React.createElement("div", { id: "layout-content-xq", style: {
                        ...
                        React.createElement("div", { style: { minWidth: 1180, height: '100%' } }, React.cloneElement(this.props.children, 
                        // 注入设置面包屑的接口
                        {
    ...
    // 删除面包屑参数
    MenuRoot.prototype.deleteBreadcrumb = function (deleteKeys) {
        var _this = this;
        if (deleteKeys === void 0) { deleteKeys = []; }
        if (this.props.navType === 'tab') {
            var nextBreadcrumb_1 = [];
            deleteKeys.forEach(function (path) {
                _this.props.breadcrumb.forEach(function (item) {
                    if (item.path !== path) {
                        nextBreadcrumb_1.push(item);
                    }
                });
            });
            this.props.setBreadcrumb(nextBreadcrumb_1);
        }
    };
    // 设置面包屑参数
